    ...
    action = wezterm.action_callback(function(window, pane)
      if KeymapUtil.is_in_tmux(pane) then
        ...
        KeymapUtil.spawn_child_process {
          ...
          "lazygit",
          "--use-config-file",
          wezterm.home_dir .. "/.config/lazygit/config.yml," .. wezterm.home_dir .. "/.config/lazygit/theme/fla.yml",
        }
      end
    end),
  },
      ...
      if KeymapUtil.is_in_tmux(pane) then
        ...
        if KeymapUtil.is_in_nvim(pane) then
          ...
          if is_zoomed then
            ...
            if add_size > 0 then
              window:perform_action(wezterm.action.AdjustPaneSize { adjust_pane_direction, add_size }, pane)
            end
          else
