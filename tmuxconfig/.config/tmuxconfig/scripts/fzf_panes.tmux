...
do_action() {
	...
	selected=$(
		FZF_DEFAULT_COMMAND=$cmd fzf -m --preview="$preview_cmd" \
			--preview-window='down:50%:nohidden' --reverse --info=inline --header-lines=1 \
			--header="Ctrl-r:reload,Ctrl-v:vertpane,Ctrl-s:splitpane,Ctrl-t:swap session,Ctrl-o:open" \
			...
			--bind="ctrl-o:execute($HOME/.local/bin/fz_ctrlo callme)+accept" \
			...
			if [[ $id == "$pane_id" ]]; then
				...
				while read -r ps_line; do
					...
					if [[ $tty == ${p_info[5]} ]]; then
						...
						printf "%-6s  %-18s  %5s  %8s  %4s  %4s  %5s  %-8s  %-7s  " \
