...
case $1 in
"btop")
	bspc rule -a \* -o state=floating center=true rectangle=1200x800+0+0 && "$TERMINAL" -e btop
	;;
"cpu")
	bspc rule -a \* -o state=floating center=true rectangle=1000x500+0+0 && "$TERMINAL" -e watch free
	;;
"bar")
	...
	if [ ! -f "$lock" ]; then
		...
		sleep 0.4
		$eww open bar && xdo lower -N eww-bar
	fi
	;;
"calendar")
