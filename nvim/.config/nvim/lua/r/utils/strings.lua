...
local function get_click_start(func_name, id)
    if not id then
        vim.schedule(function()
            local msg = fmt(
                "An ID is needed to enable click handler %s to work",
                func_name
            )
...
local function component(opts)
    ...
    assert(
        vim.tbl_islist(item),
        ("component options are required but got %s instead"):format(
            vim.inspect(item)
        )
    )
...
function M.display(sections, available_space)
    local components = as.fold(function(acc, section, count)
        ...
        as.foreach(function(args, index)
            ...
            local ok, str = as.pcall(
                ("Error creating component: %s"):format(vim.inspect(args)),
                component,
                args
            )
            if not ok then
