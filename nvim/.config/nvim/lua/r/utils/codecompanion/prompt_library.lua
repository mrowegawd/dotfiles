...
local function load_prompt_library()
  ...
  if not (stat and stat.type == "directory") then
    vim.notify(("Prompt directory not found: %s"):format(PROMPT_LIBRARY_CONFIG.prompt_dir), vim.log.levels.WARN)
    return {}
  end
...
local function helpful_assistant_prompt()
  return build_prompt("chat", "Act as a helpful assistant.", "assistant_role", M.prompt "helpful_assistant")
end
...
local function writer_at_work_prompt()
  return build_prompt("chat", "Write the way I write at work.", "writer", M.prompt "writer_at_work", {
    context = {
      {
        type = "file",
        path = {
...
local function meeting_copilot_prompt()
  return build_prompt(
    ...
    "meeting_role",
    string.format("%s\n\nToday is: %s", M.prompt "meeting_copilot", os.date "%d/%m/%Y")
  )
end

local function slides_generator_prompt()
  return build_prompt("chat", "Act as a slides strategy writing assistant.", "slides_role", M.prompt "slides_generator")
end
