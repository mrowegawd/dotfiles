...
function M.blame_line(opts)
  ...
  local root = RUtils.root.detectors.pattern(0, { ".git" })[1] or "."
...
function M.browse()
  ...
  local function open(remote)
    if remote then
      RUtils.info(("Opening [%s](%s)"):format(remote.name, remote.url))
  ...
  if #remotes == 0 then
    return RUtils.error "No git remotes found"
