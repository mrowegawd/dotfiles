...
local function get_all_commits_of_this_file()
  local scripts = vim.api.nvim_exec2("!git log --pretty=oneline --abbrev-commit --follow %", { output = true })
...
local function isFugitiveFile()
  local filepath = vim.api.nvim_exec2("echo expand('%:p')", { output = true })
  return filepath.output:find("fugitive", 1, true) == 1
end
...
local function diffWith()
  ...
  if isFugitiveFile() then
    vim.ui.input({
      prompt = "Please input commit hash id to compare with current version: ",
    }, function(input)
