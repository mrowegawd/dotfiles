...
---@return boolean ok, r.utils.treesitter.Health health
function M.check()
  ...
  ---@class r.utils.treesitter.Health: table<string,boolean>
  local ret = {
...
function M.build(cb)
  M.ensure_treesitter_cli(function(_, err)
    ...
    if ok then
      ...
      RUtils.error(lines, { title = "RUtils Treesitter" })
    end
  end)
end
...
function M.ensure_treesitter_cli(cb)
  ...
  mr.refresh(function()
    ...
    if not p:is_installed() then
      RUtils.info "Installing `tree-sitter-cli` with `mason.nvim`..."
      p:install(
        nil,
        vim.schedule_wrap(function(success)
          if success then
            RUtils.info "Installed `tree-sitter-cli` with `mason.nvim`."
            cb(true)
          else
