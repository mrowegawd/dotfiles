...
---@class r.utils.extras
local M = {}
M.buf = 0

---@type LazyExtraSource[]
M.sources = {
  { name = "RUtils", desc = "RUtils extras", module = "r.plugins.extras" },
...
function M.wants(opts)
  ...
  if opts.root then
    ...
    return #RUtils.root.detectors.pattern(M.buf, opts.root) > 0
  end
  return false
end
...
function M.get()
  ...
  for _, source in ipairs(M.sources) do
    local root = RUtils.find_root(source.module)
    if root then
      RUtils.walk(root, function(path, name, type)
...
function M.get_extra(source, modname)
  RUtils.plugin.handle_defaults = false
  ...
  return {
    ...
    managed = vim.tbl_contains(RUtils.config.json.data.extras, modname) or not enabled,
    plugins = plugins,
    optional = optional,
  }
end
...
function X:toggle()
  ...
  for _, extra in ipairs(self.extras) do
    if extra.row == pos[1] then
      ...
      RUtils.config.json.data.extras = vim.tbl_filter(function(name)
        return name ~= extra.module
      end, RUtils.config.json.data.extras)
      M.state = vim.tbl_filter(function(name)
        return name ~= extra.module
      end, M.state)
      if extra.enabled then
        table.insert(RUtils.config.json.data.extras, extra.module)
        M.state[#M.state + 1] = extra.module
      end
      table.sort(RUtils.config.json.data.extras)
      RUtils.json.save()
      RUtils.info(
        "`"
          .. extra.name
...
function X:extra(extra)
  local defaults = RUtils.config.get_defaults()
  ...
  if not extra.managed then
    ...
    elseif vim.tbl_contains(RUtils.plugin.core_imports, extra.module) or origin == "default" then
      self:diagnostic {
  ...
  if extra.source.name ~= "RUtils" then
  ...
  for _, import in ipairs(extra.imports) do
    import = import:gsub("^RUtils.plugins.extras.", "")
