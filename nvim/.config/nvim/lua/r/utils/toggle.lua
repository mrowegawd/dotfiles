...
function M.wrap(toggle)
  return setmetatable(toggle, {
    __call = function()
      ...
      if state then
        RUtils.info("Enabled " .. toggle.name, { title = toggle.name })
      else
        RUtils.warn("Disabled " .. toggle.name, { title = toggle.name })
      end
      return state
    end,
  })
...
function M.format(buf)
  return M.wrap {
    ...
    get = function()
      ...
      return RUtils.format.enabled()
    end,
    set = function(state)
      RUtils.format.enable(state, buf)
    end,
  }
end
