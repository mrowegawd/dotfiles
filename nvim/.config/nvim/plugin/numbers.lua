...
local number_buftype_ignored = { "quickfix", "prompt", "outline", "aerial" }
...
local function is_blocked()
  ...
  local decs = ui.decorations.get {
    ft = vim.bo.ft,
    bt = vim.bo.bt,
    setting = "number",
  }

  return decs ~= nil and decs.ft == false or decs ~= nil and decs.bt == false
end
...
as.augroup("ToggleRelativeLineNumbers", {
  event = { "BufEnter", "FocusGained", "InsertLeave", "BufRead" },
